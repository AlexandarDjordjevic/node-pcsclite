...
PCSCLite::PCSCLite() : m_card_context(0), m_card_reader_state(), m_status_thread(0), m_state(0) {
    assert(uv_mutex_init(&m_mutex) == 0);
    assert(uv_cond_init(&m_cond) == 0);

    LONG result = SCardEstablishContext(SCARD_SCOPE_SYSTEM, NULL, NULL, &m_card_context);
    ...
    while (!pcsclite->m_state) {
        ...
        if (result == SCARD_S_SUCCESS) {
            if (pcsclite->m_pnp) {
                ...
                result = SCardGetStatusChange(pcsclite->m_card_context, INFINITE,
                                              &pcsclite->m_card_reader_state, 1);
...
LONG PCSCLite::get_card_readers(PCSCLite* pcsclite, AsyncResult* async_result) {
    ...
    result =
      SCardListReaders(pcsclite->m_card_context, NULL, (LPTSTR)&readers_name, &readers_name_length);
#else
