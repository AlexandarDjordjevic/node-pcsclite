...
CardReader.prototype.get_status = function (reader_name, reader_name_len, state, protocol, attr, attr_len, cb) {
    if (!this.connected) {
        return cb(new Error("Card Reader not connected"));
    }

    var reader_name = new Buffer.alloc(reader_name_len);
    var attr = new Buffer.alloc(attr_len);
    this._get_status(reader_name, reader_name_len, state, protocol, attr, attr_len,
