    ...
    process.nextTick(function () {
        p.start(function (err, data) {
            ...
            new_names.forEach(function (name) {
                ...
                readers[name] = r;
                r.get_status_change(function (err, state, atr) {
                    if (err) {
...
CardReader.prototype.reconnect = function (options, cb) {
    ...
    if (typeof options.initialization === 'undefined' || options.initialization === null) {
        options.initialization = this.SCARD_LEAVE_CARD;
    }
...
CardReader.prototype.get_status = function (cb) {
    if (!this.connected) {
        return cb(new Error("Card Reader not connected"));
    }

    const READER_NAME_MAX_LEN = 1024;
    const ATTR_BUFFER_SIZE = 255;

    var reader_name = new Buffer.alloc(READER_NAME_MAX_LEN);
    var attr = new Buffer.alloc(ATTR_BUFFER_SIZE);

    this._get_status(reader_name, READER_NAME_MAX_LEN, attr, ATTR_BUFFER_SIZE,
        (err, reader_name_len, state, protocol, attr_len) => {
            if (err) {
                return cb(err);
            } else {
                cb(err, reader_name.slice(0, reader_name_len), state, protocol, attr.slice(0, attr_len));
            }
        });
};
